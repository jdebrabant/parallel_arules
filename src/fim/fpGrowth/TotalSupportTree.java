...
public class TotalSupportTree extends AssocRuleMining {
	...
	protected BufferedWriter out_file; 
	...
	public void emitFrequentSets(OutputCollector<Text,DoubleWritable> output)
	{
		try 
		{
			int number = 1;
	...
	public int emitFrequentSets(OutputCollector<Text,DoubleWritable> output, int number, String itemSetSofar, int size, TtreeNode[] linkRef)
	{
		try 
		{
			// no more nodes
	...
	public void writeFrequentSets()
	{
		try 
		{
			...
			if(output_file == null)  
			{
				System.out.println("WARNING: No output file specified, using default."); 
				output_file = "frequent_itemsets.txt"; 
			}
			
			// open the file passed as a command line argument with -O flag
			BufferedWriter out_file = new BufferedWriter(new FileWriter(output_file)); 
			...
			for (short index=1; index <= numOneItemSets; index++) 
			{
				if (startTtreeRef[index] !=null) 
				{
					...
					{
						String itemSetSofar = new Short(reconvertItem(index)).toString();
						out_file.write("{" + itemSetSofar + "} = " + startTtreeRef[index].support + "\n");
						number = writeFrequentSets(out_file, number+1,itemSetSofar, index,startTtreeRef[index].childRef);
					}
				}
			}
	...
	public int writeFrequentSets(BufferedWriter out_file, int number, String itemSetSofar, int size, TtreeNode[] linkRef)
	{
		try 
		{
			// no more nodes
			...
			for (short index=1; index < size; index++) 
			{
				if (linkRef[index] != null) 
				{
					...
					{
						String newItemSet = itemSetSofar + (reconvertItem(index));
						out_file.write("{" + newItemSet + "} = " + linkRef[index].support + "\n");
						number = writeFrequentSets(out_file, number + 1,newItemSet, index, linkRef[index].childRef);
					}
				}
			}
		}
		...
		for (short index=1; index < size; index++) {
			if (linkRef[index] != null) {
				if (linkRef[index].support >= minSupport) {
					...
					System.out.println("{" + newItemSet + "} = " + linkRef[index].support);
					number = outputFrequentSets(number + 1,newItemSet, index, linkRef[index].childRef);
	            }
			}
	    }
